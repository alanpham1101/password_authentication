...
class HashingStoring(PasswordStoring, HashFunction):
    def update_password(self, db, user, hash_fn: Callable[[bytes], bytes], password: str) -> None:
        hashed_password = self.hash_str_and_b64_encode(hash_fn, password)
    ...
    def verify_password(self, user, password: str) -> None:
        ...
        hashed_pw = self.hash_str_and_b64_encode(hash_fn, password)
